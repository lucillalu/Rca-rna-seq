...
def get_single_read(fp):
	...
	next_char = peek(fp, 1).decode('ascii');
	num_loaded_lines = 1;
	...
	while (num_loaded_lines < num_expected_lines and len(next_char) > 0):
		line = fp.readline().strip().decode('ascii');
		next_char = peek(fp, 1);
		if (next_char != ''):
			next_char = next_char.decode('ascii');
		...
		if (header_leading_char == '@'):
			if ((num_loaded_lines == 1 and next_char == '+') or
				...
				(num_loaded_lines == 3 and next_char == sequence_separator)):
					line_string += line.rstrip().decode('ascii');
					lines.append(line_string);
					num_loaded_lines += 1;
					line_string = '';
